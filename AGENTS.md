...
> **这个仓库不再接受改动。** 继任者是 [itswl/newsdesk](https://github.com/itswl/newsdesk)
> （Go + TypeScript）。要改行为就去那边改，这边只做参考。
>
> 下面这些约束**全部仍然成立**，只是文件名换了——它们不是 Python 的约束，是这条
> 流水线的约束（用量窗口、沙箱边界、告警的四层、方法论上的坑）。继任仓库的
> `AGENTS.md` 是同一份内容改写的，两边对照着看就知道哪些是实现细节、哪些是本质。
