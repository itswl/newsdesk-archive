...
| `SITE_LANGS` | 发布哪些语言（逗号分隔，第一个放根目录）。留空 = 只发简体，见下 |
...
```
/              简体   模型直接产出
/zh-TW/        繁体   OpenCC s2twp 现转
/en/           英文   bin/translate.py 预先翻好
```

三者的代价完全不是一个量级，所以实现方式也不同：

**繁体近乎免费。** OpenCC `s2twp` 做的是**台湾用词**转换而不是简单换字——软件→軟體、
网络→網路、内存→記憶體、鼠标→滑鼠、缓存→快取。24 KB 报告转换 0.02 秒，零模型调用。
所以它**不落盘**：构建时现转，存一份反而多一处会过期的副本。界面文案同理，`i18n.py`
里只手写简中与英文两份，繁中推导出来。

**英文是真翻译。** `bin/translate.py` 直接调 API（不走 agentic 循环——翻译是纯转换，
不需要工具也不需要沙箱），用 `FALLBACK_*` 那套按量付费配置，**不吃订阅的用量窗口**。
每天约 10 万 token，单篇 40–90 秒。产出落在 `reports/<日期>/<名字>.en.md`，
幂等：源文件没变就跳过。

翻译失败不拖垮流水线——中文站照常发布，英文那份缺一篇，页面显示「当天没有这份报告」。

**结构比措辞更要紧。** 译文最危险的失败不是翻得不好，是表格列数变了或输出被
`max_tokens` 截断——两者都不报错，页面照常渲染，只是塌了或少了后半截。所以
`translate.py` 会比对结构指纹（表格行、链接数、标题层级、代码围栏）并检测截断。
