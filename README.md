...
有三类失败**重试帮不上忙**，它们共同点是「等 10 分钟再来一次必然还是失败」：

| 类别 | 典型报错 | 该做的事 |
|---|---|---|
| 配额耗尽 | `usage limit reached` / `429` | 等窗口重置，或换额度更宽的引擎 |
| 余额不足 | `Insufficient Balance` / `余额不足` / `402` | 去供应商后台充值——靠等不会好 |
| 凭据失效 | `invalid api key` / `401` | 换 key，重试到天亮也没用 |

这三类都跳过常规重试（用量窗口 5 小时，`RETRY_GAP_MIN` 才 10 分钟，重试时窗口根本
没重置）。分开认是为了告警能给出**该做的事**，而不是只说「失败了」——余额不足被报成
配额耗尽的话，提示会是「等窗口重置」，而那是永远等不到的。
...
按量付费**不受订阅的 5 小时窗口约束**——主引擎因为窗口没重置而跑不动时，它不受这条
限制。但它不是保底：API 侧有自己的失败方式（余额不足、限流、服务故障、key 失效），
换的是一个**独立的失败源**，不是一个保证。所以余额要留意，`ENGINE_FALLBACK` 也不能
当成「配了就不会断」。

三个变量缺一个就拒绝运行，不会静默退回没有兜底的状态。`schedulerctl.sh status` 会显示
兜底具体指向哪个模型和主机——「配了但指向哪儿」是最容易配错又看不出来的地方。
...
**feed 跟着同一个窗口走。** 条数由 `PUBLIC_DAYS × 每天篇数` 算出来，不是另设一个上限
——以前写死 20，满负荷时 7 天 × 4 篇 = 28 条，feed 会悄悄收窄到 5 天而站点还是 7 天。
构建输出里会报「覆盖 N 天 / 站点 N 天」，不一致时明确标出来。7 天全文大约 500 KB。
...
- **覆盖范围与站点一致**，由 `PUBLIC_DAYS` 一个旋钮决定（条数 = 天数 × 每天篇数），
  不另设固定上限——写死条数的话，满负荷时 feed 会比站点窄，而且不报错。7 天全文约
  500 KB；正文用 CDATA 而不是 XML 转义，否则每个 `<` 变成 `&lt;`，体积近乎翻倍。
- 页面在 `PUBLIC_DAYS` 之后会下线，但阅读器里已抓下的全文仍在——只是其中的链接会
  指向已失效的页面。这是保留期的固有结果，不是 bug。
